...
const ignoreUrlFilter = (results, ignoreFileName) => {
  ...
  fs.readFile(ignoreFileName, 'utf8', (err, data) => {
    if (err) console.log(err);
      ...
      var invalid = data.match(/[\n][^(#|http|https)].+$/gm);
      if (invalid == null) {
        var urlsIgnore = data.match(/(http|https)(:\/\/)([\w+\-&@`~#$%^*.=/?:]+)/gi);
        results.forEach((result) => {
          if (!urlsIgnore.includes(result.url)) resultsFinal.push(result);
        });
        displayAll = true;
        displayGood = true;
        displayBad = true;
      ...
      Promise.all(promises)
        .then((results) => {
          if (process.argv[3] === '-j' || process.argv[3] === '--json' || process.argv[3] === 'j')
            ...
            if (ignoreFileName) ignoreUrlFilter(results, ignoreFileName);
            else
              console.log(
                chalk.red.bold(
                  `Please provide this pattern: urltester <filename> -i/--ignore <filename> `
                )
              );
          } else {
            displayAll = true;
